...
import { ActivityTimelineEntry, getActivityTimeline } from '../../services/activities-api';
...
export function ActivityTimeline() {
  ...
  useEffect(() => {
    async function load() {
      if (!session || !organizationId || !activityId) {
        return;
      }
      setLoading(true);
      try {
        setEntries(await getActivityTimeline(session.accessToken, organizationId, activityId));
      } catch (err) {
