...
import { getLeadTimeline } from '../../services/leads-api';
...
export function LeadTimeline() {
  ...
  useEffect(() => {
    async function load() {
      ...
      setLoading(true);
      try {
        // Las Activities de este Lead se agregan automáticamente (FR-009 de
        // spec 012, research.md #13).
        const [leadEntries, activitiesResult] = await Promise.all([
          getLeadTimeline(session.accessToken, organizationId, leadId),
          searchActivities(session.accessToken, organizationId, { leadId }),
        ]);
        ...
        const merged = [...(leadEntries as TimelineEntryLike[]), ...activityEntries].sort(
