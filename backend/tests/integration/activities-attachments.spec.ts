...
describe('Adjuntar un archivo a una Activity (US3, Acceptance Scenario 1, RN-007)', () => {
  ...
  it('queda disponible mientras exista la Activity, y sobrevive a su cancelación', async () => {
    const owner = await registerAndLogin('activities-attachments-owner@example.com');
    const token = owner.body.accessToken as string;
    const org = await request(app.getHttpServer())
      .post('/api/v1/organizations')
      .set('Authorization', `Bearer ${token}`)
      .send({ name: 'Activities Attachments Org', timezone: 'UTC', currency: 'USD', language: 'en' })
      .expect(201);
    ...
    const activity = await request(app.getHttpServer())
      ...
      .set('Authorization', `Bearer ${token}`)
      .set('X-Organization-Id', organizationId)
      .send({ customerId: customer.body.id, activityTypeId: nota.id, title: 'Nota con adjunto', scheduledAt: new Date().toISOString() })
      .expect(201);
    ...
    expect(list1.body[0]).toMatchObject({ fileName: 'propuesta.pdf', fileUrl: 'https://files.example.com/propuesta.pdf' });
